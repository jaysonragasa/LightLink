...
public sealed partial class MauiCameraView : UserControl, IDisposable
{
    ...
    private async Task ProcessFrameAsync(MediaFrameReference frame)
    {
        ...
        // Get raw pixel bytes
        byte[] pixelData = await GetRawPixelsAsync(softwareBitmap);

        // 🔥 Send frame to CameraView (you can make an event for this)
        cameraView.OnFrameReceived(pixelData, softwareBitmap.PixelWidth, softwareBitmap.PixelHeight);
    ...
    byte[] rawPixels = null;
    bool gettingRawPixels = false;
    private async Task<byte[]> GetRawPixelsAsync(SoftwareBitmap softwareBitmap)
    {
        if (gettingRawPixels)
            return rawPixels;

        gettingRawPixels = true;
        if (softwareBitmap == null)
            return null;

        // Convert to a known format (BGRA8 is commonly used)
        using var convertedBitmap = SoftwareBitmap.Convert(softwareBitmap, BitmapPixelFormat.Bgra8, BitmapAlphaMode.Premultiplied);
        rawPixels = await EncodedBytes(convertedBitmap, BitmapEncoder.JpegEncoderId);

        gettingRawPixels = false;
