	...
	internal ImageSource GetSnapShot(ImageFormat imageFormat, bool auto = false)
	{
		...
		if (started && !snapping)
		{
			...
			if (bitmap != null)
			{
				...
				byte[] argbBytes = new byte[intPixels.Length * 4];
				for (int i = 0; i < intPixels.Length; i++)
				{
					int color = intPixels[i];
					argbBytes[i * 4 + 0] = (byte)((color >> 24) & 0xFF); // A
					argbBytes[i * 4 + 1] = (byte)((color >> 16) & 0xFF); // R
					argbBytes[i * 4 + 2] = (byte)((color >> 8) & 0xFF);  // G
					argbBytes[i * 4 + 3] = (byte)(color & 0xFF);         // B
				}

				// 🔹 Send to CameraView handler
				cameraView?.OnFrameReceived(argbBytes, width, height);

				// 🔹 Encode and return ImageSource
				var iformat = imageFormat switch
				{
					ImageFormat.JPEG => Bitmap.CompressFormat.Jpeg,
					_ => Bitmap.CompressFormat.Png
